...
app = Flask(__name__)
app.secret_key = "dm-linker-v10-browser-lookup"
...
<body>
  <div class="card">
    <h1>DM Linker V10</h1>
    <p class="muted">Browser-lookup: appen öppnar Jula-sökningen som en riktig browser, hittar produktkort och länkar bara riktiga produktsidor. Ingen search/fallback skapas.</p>
...
<body>
  <div class="card">
    ...
    <div class="ok">
      ...
      Unika artiklar hittade: <strong>{{ total_articles }}</strong>.<br>
      Artiklar utan säker produktsida: <strong>{{ missing|length }}</strong>.
    </div>
    {% if missing %}
      <div class="warn">
        <strong>Vissa produkter fick ingen länk.</strong><br>
        Orsak: appen hittade ingen säker produktsida via browser-lookup. Ingen fallback/search-länk har skapats.
      </div>
...
def browser_lookup_urls(country: str, articles: list[str]) -> dict:
    """Lookup with a real Chromium browser. Returns article -> product URL or None."""
    base = COUNTRY_BASE[country]
    ...
    with sync_playwright() as p:
        ...
        for article in articles:
            ...
            # Try to close cookie banner if present, ignore if absent.
            ...
            # First strict pass: URL itself contains article number.
            for href in hrefs:
            ...
            # Second pass: click first catalog candidate and verify page text contains article.
            # This handles cases where product card href is present but article appears after redirect.
            for href in hrefs[:8]:
                clean = href.split("?")[0].split("#")[0]
...
@app.get('/health')
def health():
    return {'ok': True, 'version': 'v10-browser-lookup'}
